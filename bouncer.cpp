...
class Bouncer : public CModule {
	public:
		...
		CModule::EModRet OnUserRaw(CString &sLine) {
...
template <>
void TModInfo<Bouncer>(CModInfo& Info) {
    Info.SetWikiPage("bouncer");
}

GLOBALMODULEDEFS(Bouncer, t_s("Provides KiwiIRC BOUNCER support"))
